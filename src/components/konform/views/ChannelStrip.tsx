...
export const ChannelStrip = ({
  ...
  return (
    <div 
      className={`group relative flex flex-col bg-black/20 rounded-lg p-2 gap-2 ${className || ''}`}
      style={{ 
        width: style?.width || '120px',
        borderLeft: `2px solid ${channel.color || '#00D1FF'}`, 
        ...style 
      }}
    >
      {/* Context Menu */}
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          {onColorChange && (
            <>
              <DropdownMenuItem onClick={() => onColorChange('#00D1FF')}>Blue</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onColorChange('#FF5F1F')}>Orange</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onColorChange('#10B981')}>Green</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onColorChange('#8B5CF6')}>Purple</DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
      </DropdownMenu>
