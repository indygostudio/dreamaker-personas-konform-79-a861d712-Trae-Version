// Script to transfer all personas to a specific user account
...
async function transferAllPersonas() {
  ...
  try {
    ...
    if (userError) {
      ...
      // Fallback: Try to find the user in profiles table by email
      ...
      var targetUserId = profileData.id;
    } else {
      if (!userData?.user) {
        throw new Error(`User with email ${targetEmail} not found`);
      }
      
      var targetUserId = userData.user.id;
    }
