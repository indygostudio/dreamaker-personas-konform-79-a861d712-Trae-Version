...
const createVerifiedArtists = async () => {
  for (const artist of verifiedArtists) {
    ...
    if (!existingArtist) {
      ...
      if (error) {
        ...
        console.log(`Created profile for ${artist.username}`);
      }
    } else {
      console.log(`Profile already exists for ${artist.username}`);
    }
  }
};

createVerifiedArtists().catch(console.error);