...
import CreateSceneForm from "./CreateStoryForm"; // We'll rename this file reference but keep the actual file name
import SceneList from "./StoryList"; // We'll rename this file reference but keep the actual file name
import DeleteSceneDialog from "./DeleteStoryDialog"; // We'll rename this file reference but keep the actual file name
...
const SceneEditor = () => {
  ...
  return <div className="space-y-6">
      <div className="flex items-center gap-2">
        {isEditingProjectName ? <Input ref={projectNameInputRef} value={newProjectName} onChange={e => setNewProjectName(e.target.value)} onKeyDown={handleProjectNameKeyDown} onBlur={handleSaveProjectName} className="max-w-[300px] bg-black/70 border-white/10 text-white h-9" /> : <h2 className="text-2xl font-semibold cursor-pointer hover:text-yellow-300 transition-colors" onDoubleClick={handleStartEditProjectName} title="Double-click to rename project">
            {activeProject.name}
          </h2>}
      </div>
      ...
      <DeleteSceneDialog isOpen={!!sceneToDelete} onClose={() => setSceneToDelete(null)} onConfirm={handleDeleteScene} />
    </div>;
};
