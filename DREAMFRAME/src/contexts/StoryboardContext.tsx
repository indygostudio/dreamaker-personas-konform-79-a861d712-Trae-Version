...
export const StoryboardProvider: React.FC<StoryboardProviderProps> = ({ children }) => {
  ...
  const dispatchWithUpdates = (action: StoryboardAction) => {
    ...
    if (action.type === "ADD_STORY") {
      ...
      // Dispatch the action with the ID included
      dispatch({
        type: "ADD_STORY",
        payload: {
          ...action.payload,
    ...
    if (action.type === "ADD_SCENE") {
      // For ADD_SCENE, we need to generate and return a new scene ID
    ...
    // For all other actions, just dispatch
    dispatch(action);
  };
